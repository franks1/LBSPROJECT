...
namespace GetLabourManager
{
    ...
    public class ApplicationUserManager : UserManager<ApplicationUser, int>
    {
        ...
        public static ApplicationUser FindUserByName(RBACDbContext db, string _userId)
        {
            ApplicationUser _retVal = null;
            try
            {
                _retVal = db.Users.Where(p => p.UserName == _userId).
                    Include("Roles").Include(x => x.Roles.Select(r => r.Role.PERMISSIONS)).FirstOrDefault();
            }
            catch (Exception)
            {
            }
        ...
        public static List<ApplicationUser> GetUsers()
        {
            ...
            try
            {
                ...
                _retVal = db.Users.Select(c => c).Include("Roles").OrderBy(r => r.Lastname).ThenBy(r => r.Firstname).ToList();
            }
            catch (Exception)
            {
            }
        ...
        public static bool UpdateUser(ApplicationUserViewModel _user)
        {
            bool _retVal = false;
            try
            {
                using (RBACDbContext db = new RBACDbContext())
                {
                    ...
                    db.Entry(_user2Modify).Entity.Inactive = _user.UserStatus == "ACTIVE" ? true : false;
        ...
        public static List<ApplicationUser> GetUsers4SelectList()
        {
            ...
            try
            {
                RBACDbContext db = new RBACDbContext();
                // {
                _retVal = db.Users.Where(r => r.Inactive == false || r.Inactive == true).ToList();
                //   }
            }
            catch (Exception)
            {

...
namespace GetLabourManager.ActionFilters
{
    public class RBACAttribute : AuthorizeAttribute
    {
        public override void OnAuthorization(AuthorizationContext filterContext)
        {
            Type action_desc = null;
            ...
            try
            {
                ...
                {
                 //    action_desc = filterContext.ActionDescriptor.ControllerDescriptor
                //        .ControllerType.GetMethod (filterContext.ActionDescriptor.ActionName).ReturnType;


                  var  method_set =filterContext.ActionDescriptor.ControllerDescriptor
                       .ControllerType.GetMethods();
                    
                    foreach (var item in method_set)
                    {

                        if (filterContext.ActionDescriptor.ActionName == item.Name)
                        {
                            action_desc = item.ReturnType;
                            break;
                        }
                    }

                    if (action_desc.FullName.Contains("System.Web.Mvc.JsonResult")) return;
